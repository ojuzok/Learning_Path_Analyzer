...
def extract_student_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for student_id in df["student_id"].unique():
        ...
        feature_dict = {
            ...
            "quiz_attempts": len(
                student_data[student_data["event_type"] == "quiz_attempt"]
            ),
            "forum_posts": len(
                student_data[student_data["event_type"] == "forum_post"]
            ),
            "assignment_submissions": len(
                student_data[student_data["event_type"] == "assignment_submission"]
            ),
            "video_watches": len(
                student_data[student_data["event_type"] == "video_watch"]
            ),
        }
...
def plot_correlation_heatmap(
    corr_df: pd.DataFrame, output_path: str = "reports/correlation_heatmap.png"
):
    """
...
def plot_student_performance(
    features_df: pd.DataFrame, output_path: str = "reports/student_performance.png"
):
    """
    ...
    axes[0, 0].hist(
        features_df["avg_grade"], bins=10, edgecolor="black", color="skyblue"
    )
    ...
    axes[0, 1].scatter(
        features_df["quiz_attempts"],
        features_df["avg_grade"],
        alpha=0.6,
        s=100,
        color="coral",
    )
...
def generate_recommendations(
    corr_df: pd.DataFrame, features_df: pd.DataFrame
) -> List[str]:
    """
    ...
    for activity, corr in grade_corr.items():
        ...
        recommendations.append(
            f"  • {activity}: {corr:.3f} ({impact} {direction} impact)"
        )
    ...
    mid_performers = len(
        features_df[(features_df["avg_grade"] >= 70) & (features_df["avg_grade"] < 85)]
    )
